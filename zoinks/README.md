...
I am sure that the intended way to solve this is with some clever maths to just calculate whether the Mystery Gang would win or not without having to simulate the whole thing. But I just wrote a quick and dirty binary search, simulating the whole fight on each value until I had a solution. At first I was doing it properly, with the fractions module to ensure that the simulations were accurate, but this was slightly too slow on the hidden test cases. Replacing the fractions with simple floating point multiplication made it run fast enough (still took over 30 seconds to run!), but it *might* have made the simulations inaccurate, so some test cases may have failed here.
